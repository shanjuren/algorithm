    ...
    public static long subArrayRanges(int[] nums) {
        ...
        for (int start = 0; start < n; start++) {
            for (int end = start; end < n; end++) {
                if(tmpMax<0 || nums[end]>nums[tmpMax]){
                    tmpMax = end;
                }
                if(tmpMin<0 || nums[end]<nums[tmpMin]){
                    tmpMin = end;
                }
               tmpSum+=(nums[tmpMax]-nums[tmpMin]);
            }
            tmpMax = -1;
            tmpMin = -1;
        }
